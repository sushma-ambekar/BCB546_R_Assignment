...
ggplot(merge_fang, aes(Chromosome)) + geom_bar(aes(x=Chromosome), color="black", fill="#FDB863") + labs(x="Chromosome",y="# of SNPs")
```
To see what groups contribute the most SNPs, we can do a simple plot with the original data.

```{r, echo=FALSE}
ggplot(fang_et_al_genotypes, aes(Group)) + geom_bar() + theme_classic() + theme(axis.text.x = element_text(angle = 90))
```
